      ...
      <div className="max-w-5xl mx-auto px-4 py-12">
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  <label className="text-xs font-black uppercase tracking-[0.15em] text-slate-400 ml-2">Message du client *</label>
                ...
                <div className="space-y-4">
                   <label className="text-xs font-black uppercase tracking-[0.15em] text-slate-400 ml-2">Nature du conflit *</label>
              ...
              {hasUsedFree && !isAdmin ? (
                <div className="bg-[#0F172A] p-10 rounded-[2.5rem] text-white flex flex-col md:flex-row items-center gap-8 shadow-2xl border-b-8 border-green-600 transition-transform hover:scale-[1.01]">
            ...
            <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-200">
              ...
              <div className="grid md:grid-cols-3 gap-6 mb-12 items-stretch">
                <div className="bg-slate-50 border-2 border-slate-200 rounded-[2rem] p-8 flex flex-col transition-all hover:bg-white hover:border-red-200 group">
                   ...
                   <ul className="text-xs space-y-4 mb-8 text-slate-500 font-bold leading-relaxed">
                ...
                <div className="bg-slate-50 border-2 border-slate-200 rounded-[2rem] p-8 flex flex-col transition-all hover:bg-white hover:border-blue-200 group">
                   <Scale className="w-10 h-10 text-slate-400 mb-6 group-hover:text-blue-500" />
        ...
        {step === 'paid-result' && (
           <div className="space-y-8 animate-in zoom-in-95 duration-700">
             <div className="bg-white rounded-[3rem] shadow-2xl p-10 border-t-8 border-green-600">
               <div className="flex items-center gap-5 mb-10 text-[#1E293B]"><Check className="w-14 h-14 text-green-500" /><h2 className="font-black text-4xl tracking-tight">Réponse Sécurisée</h2></div>
               <div className="bg-slate-50 p-10 rounded-[2.5rem] border-2 border-slate-100 text-slate-800 leading-relaxed font-serif text-2xl mb-12 shadow-inner whitespace-pre-wrap italic">{paidResponse}</div>
               <button onClick={() => { navigator.clipboard.writeText(paidResponse); alert('Copié !'); }} className="w-full bg-[#1E293B] text-white font-black py-7 rounded-2xl hover:bg-black transition-all shadow-xl flex items-center justify-center gap-6 text-2xl">
           ...
           <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-lg shadow-2xl relative border-t-[12px] border-blue-600">
              ...
              {supportSent ? (<div className="text-green-600 text-center py-12"><Check className="w-24 h-24 mx-auto mb-8 animate-pulse" /><p className="font-black text-3xl tracking-tight uppercase">Envoyé !</p></div>) : (
                <form onSubmit={async (e) => { e.preventDefault(); setLoading(true); await fetch('/api/chat', { method: 'POST', body: JSON.stringify({ type: 'feedback', name: supportName, email: supportEmail, message: supportMessage }) }); setSupportSent(true); setTimeout(() => setShowSupport(false), 2000); }} className="space-y-6">
                  ...
                  <textarea required rows={4} placeholder="Votre question..." className="w-full bg-slate-50 border-2 border-slate-100 p-6 rounded-2xl focus:border-blue-600 outline-none font-bold text-lg" value={supportMessage} onChange={e => setSupportMessage(e.target.value)} />
                  <button type="submit" className="w-full bg-[#1E293B] text-white font-black py-6 rounded-2xl hover:bg-black transition-all uppercase tracking-widest shadow-2xl text-xl">Envoyer</button>
                </form>
              )}
           </div>
        </div>
