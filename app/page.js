    ...
    <div className="min-h-screen bg-[#F1F5F9] text-[#1E293B] font-sans">
      <div className="max-w-5xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-sm md:text-base text-slate-400 italic mt-4 font-bold">
            ...
            <div className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-200">
              ...
              <div className="grid md:grid-cols-3 gap-6 mb-12 items-stretch">
                <div className="bg-slate-50 border-2 border-slate-200 rounded-[2rem] p-8 flex flex-col transition-all hover:bg-white hover:border-red-200 group">
                   ...
                   <h4 className="font-black text-xl mb-4 text-slate-700 text-center">Seul</h4>
        ...
        {step === 'paid-result' && (
           <div className="space-y-8 animate-in zoom-in-95 duration-700 text-center">
             <div className="bg-white rounded-[3rem] shadow-2xl p-10 border-t-8 border-green-500">
               <div className="flex items-center gap-5 mb-10 text-[#1E293B] justify-center"><Check className="w-14 h-14 text-green-500" /><h2 className="font-black text-4xl tracking-tight italic text-center">Réponse Sécurisée</h2></div>
               <div className="bg-slate-50 p-10 rounded-[2.5rem] border-2 border-slate-100 text-slate-800 leading-relaxed font-serif text-2xl mb-12 shadow-inner whitespace-pre-wrap italic text-center">{paidResponse}</div>
      ...
      {showSupport && (
        <div className="fixed inset-0 bg-[#0F172A]/90 backdrop-blur-xl flex items-center justify-center p-4 z-50">
           <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-lg shadow-2xl relative border-t-[12px] border-blue-600 text-center">
              ...
              {supportSent ? (<div className="text-green-600 text-center py-12"><Check className="w-24 h-24 mx-auto mb-8 animate-pulse text-green-600" /><p className="font-black text-3xl tracking-tight uppercase">Envoyé !</p></div>) : (
                <form onSubmit={async (e) => { e.preventDefault(); setLoading(true); await fetch('/api/chat', { method: 'POST', body: JSON.stringify({ type: 'feedback', name: supportName, email: supportEmail, message: supportMessage }) }); setSupportSent(true); setTimeout(() => setShowSupport(false), 2000); }} className="space-y-6">
                  ...
                  <button type="submit" className="w-full bg-[#1E293B] text-white font-black py-6 rounded-2xl hover:bg-black transition-all uppercase tracking-widest shadow-2xl text-xl italic text-center">Envoyer</button>
                </form>
              )}
           </div>
        </div>
...
// ЭТА ФУНКЦИЯ ТЕПЕРЬ ОБЪЯВЛЕНА ТОЛЬКО ОДИН РАЗ
